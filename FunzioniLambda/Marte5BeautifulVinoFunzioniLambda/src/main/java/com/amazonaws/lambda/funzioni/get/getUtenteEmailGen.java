...
public class getUtenteEmailGen implements RequestHandler<RichiestaGetGenerica, RispostaGetGenerica> {
    ...
    private RispostaGetGenerica getRisposta(RichiestaGetGenerica input) {
		...
		String emailUtente = input.getEmailUtente();
		...
		if(client != null) {
			...
			expr1.addFilterCondition("emailUtente", new Condition().withComparisonOperator(ComparisonOperator.EQ).withAttributeValueList(attributi));
			
			List<Utente> utenti;
			try {
				utenti = mapper.scan(Utente.class, expr1);
			} catch (Exception e1) {
				esito.setCodice(EsitoHelper.ESITO_KO_CODICE_ERRORE_GET);
				esito.setMessage(this.getClass().getName() + " - " + EsitoHelper.ESITO_KO_MESSAGGIO_ERRORE_GET + " getUtenteEmail ");
				esito.setTrace(e1.getMessage());
				risposta.setEsito(esito);
				return risposta;
			}
			...
			if(utenti.size() == 0) {
				esito.setCodice(EsitoHelper.ESITO_KO_CODICE_ERRORE_GET);
				esito.setMessage(this.getClass().getName() + " - " + EsitoHelper.ESITO_KO_MESSAGGIO_ERRORE_GET + " getUtenteEmail, nessun utente trovato ");
				risposta.setEsito(esito);
				return risposta;
			}
			utente = utenti.get(0);
